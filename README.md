...
destination, and every existing planned destination before the first copy. For
COPY only, a destination inside the source is allowed after a specific warning
and explicit confirmation. Its entire existing subtree is automatically
excluded before the proposal is generated, and execution consumes that fixed
proposal, so neither existing destination media nor newly written output becomes
source material during the operation. MOVE into a destination inside the source
remains rejected. Completed files are kept if a later runtime failure occurs;
incomplete copies use distinguishable temporary files and are cleaned up on a
best-effort basis.
